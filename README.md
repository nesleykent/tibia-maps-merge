...
The interface is English-only. Static text lives in `docs/index.html`; strings
generated by `app.js` and downloadable logs live in
[`docs/lib/i18n.js`](docs/lib/i18n.js), with fixed `en-US` number and date
formatting.
...
`#edit-marks`, `#marker-sets`) so it can be linked to and survives a reload,
set with `replaceState` so a tab does not cost a press of Back. An unrecognised
...
works** is a pill in the header row; **where the client keeps
`minimapmarkers.bin`** is its
