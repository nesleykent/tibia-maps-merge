...
const STRINGS = {
  en: {
    ...
    promptTargetEmpty: 'Paste a quest article link above to load it into the prompt.',
    promptTargetInvalid: 'That is not a link yet -- the prompt needs a full http(s) address.',
    promptTargetReady: (url) => `Loaded into the prompt: ${url}`,
    promptCopied: (url) => `Prompt copied for ${url} -- paste it into ChatGPT or another assistant.`,
    promptCopyFailed: 'Could not reach the clipboard. Open the guide and copy the prompt from there.',
    promptOpened: (url) => `Opened ChatGPT with the prompt for ${url}. If it arrives empty, use Copy Prompt and paste it.`,
  ...
  'pt-BR': {
    ...
    promptTargetEmpty: 'Cole acima o link de um artigo de quest para carregá-lo no prompt.',
    promptTargetInvalid: 'Isso ainda não é um link -- o prompt precisa de um endereço http(s) completo.',
    promptTargetReady: (url) => `Carregado no prompt: ${url}`,
    promptCopied: (url) => `Prompt copiado para ${url} -- cole no ChatGPT ou em outro assistente.`,
    promptCopyFailed: 'Não foi possível acessar a área de transferência. Abra o guia e copie o prompt de lá.',
    promptOpened: (url) => `ChatGPT aberto com o prompt para ${url}. Se chegar vazio, use Copiar Prompt e cole.`,
